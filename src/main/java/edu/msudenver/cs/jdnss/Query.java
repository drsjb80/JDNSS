...
public class Query
{
	...
	buffer[1] = Utils.getByte (id, 1);
        buffer[2] = (byte)((QR ? 128 : 0) | (opcode << 3) | (AA ? 4 : 0) |
	    (TC ? 2 : 0) | (RD ? 1 : 0));
        buffer[3] = (byte)((RA ? 8 : 0) | rcode);	// put in real rcode
    	...
    	for (int i = 0; i < questions.length; i++)
	{
	    ...
	    buffer = Utils.combine (buffer, Utils.getTwoBytes (qtype[i]));
	    buffer = Utils.combine (buffer, Utils.getTwoBytes (qclass[i]));
	}
    }
